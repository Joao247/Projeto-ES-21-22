...
public class FailedAnswerService {
    ...
    public void updateFailedAnswers(int dashboardId, String startDate, String endDate) {
        ...
        if (startDate == null) start = getLastCheckDate(dashboard, now).minusSeconds(3601);
        else start = LocalDateTime.parse(startDate, DateTimeFormatter.ISO_DATE_TIME).minusSeconds(3601);
        if (endDate == null) end = now.minusSeconds(3601);
        else end = LocalDateTime.parse(endDate, DateTimeFormatter.ISO_DATE_TIME).minusSeconds(3601);
        ...
        dashboard.setLastCheckFailedAnswers(studentAnswers.stream()
                ...
                .sorted()
                .findFirst()
                .map(localDateTime -> localDateTime.minusSeconds(3601))
                .orElse(now));
    }
