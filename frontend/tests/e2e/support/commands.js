...
Cypress.Commands.add('openFailedAnswersAndDeleteFailedAnswer', () => {
  ...
  cy.get('[data-cy="deleteFailedAnswerButton"]').should('have.length.gte', 1).eq(0).click();
...
Cypress.Commands.add('openDifficultQuestionsAndDeleteDifficultQuestion', () => {
    cy.get('[data-cy="difficultQuestionsMenuButton"]').click();
    cy.get('[data-cy="refreshDifficultQuestionsButton"]').click();
    cy.intercept('DELETE', '/students/difficultquestions/*').as('deleteDifficultQuestion');
    cy.get('[data-cy="deleteDifficultQuestionButton"]').should('have.length.gte', 1).eq(0).click();
    cy.wait('@deleteDifficultQuestion');
    cy.contains('Error').parent().find('button').click();
    cy.get('[data-cy="refreshDifficultQuestionsButton"]').click();
    cy.get('[data-cy="deleteDifficultQuestionButton"]').should('have.length.gte', 1).eq(0).click();
    cy.intercept('DELETE', '/students/dashboards/*/difficultquestions').as('deleteDifficultQuestion');
});
...
Cypress.Commands.add('openWeeklyScoresAndDeleteWeeklyScore', () => {
  ...
  cy.get('[data-cy="deleteWeeklyScoreButton"]').should('have.length.gte', 1).eq(0).click();
